...
class MilkaApp {
  ...
  toggleMode() {
    this.state.currentMode =
      this.state.currentMode === "list" ? "slideshow" : "list";
    this.state.currentCardIndex = 0;
    this.render();
  }
  ...
  // 切换主题
  toggleTheme() {
    this.state.styleTheme =
      this.state.styleTheme === "minimalist-white"
        ? "night-black"
        : "minimalist-white";
    this.applyTheme();
    this.render();
...
// 在 app.js 的 MilkaApp 类中
    ...
    return `
      <div class="error-banner">
        <div class="error-content">
          ...
          <span class="error-message">${this.escapeHtml(
      ...
      <div class="themes-container">
        <div class="themes-grid">
          ${this.state.themes
            .map(
              (theme) => `
            <div class="theme-card" onclick="app.handleThemeSelect('${
              theme.id
            }')">
  ...
  getCardsListHTML() {
    ...
    return ` <div class="cards-list"> <div class="cards-grid"> ${this.state.currentCards
      .map(
        (card, index) => ` <div class="card-item" data-card-index="${index}"> 
   ...
   <div class="card-face card-front"> <div class="card-content">${this.escapeHtml(
     card.front?.main_text || "正面内容"
          ...
          card.front?.notes
            ...
            : ""
        } </div> <div class="card-face card-back"> <div class="card-content">${this.escapeHtml(
  ...
  getSlideshowHTML() {
   ...
   <div class="slideshow-counter"> ${this.state.currentCardIndex + 1} / ${
      this.state.currentCards.length
    } </div> 
   <div class="slideshow-controls"> </div> </div> 
   <div class="slideshow-card-container" data-card-index="${
     this.state.currentCardIndex
   }"> 
   <div class="slideshow-card card ${
     currentCard.isFlipped ? "flipped" : ""
   }" onclick="app.flipCard(${this.state.currentCardIndex})"> 
   <div class="card-face card-front"> <div class="card-content">${this.escapeHtml(
     currentCard.front?.main_text || "正面内容"
   )}</div> ${
      currentCard.front?.notes
        ? `<div class="card-notes">${this.escapeHtml(
            currentCard.front.notes
          )}</div>`
        : ""
    } </div> <div class="card-face card-back"> <div class="card-content">${this.escapeHtml(
      currentCard.back?.main_text || "背面内容"
    )}</div> ${
      currentCard.back?.notes
        ...
        : ""
    } </div> </div> </div> <div class="slideshow-navigation"> <button class="btn btn-secondary" onclick="app.previousCard()" ${
      this.state.currentCardIndex === 0 ? "disabled" : ""
    }> ← 上一张 </button> <button class="btn btn-secondary" onclick="app.flipCard(${
      this.state.currentCardIndex
    })"> 🔄 翻转 </button> <button class="btn btn-secondary" onclick="app.nextCard()" ${
      this.state.currentCardIndex === this.state.currentCards.length - 1
        ? "disabled"
        : ""
    }> 下一张 → </button> </div> </div> `;
  }
